 ...
 */
editCom.formateEmoji = function (str) {
...
editCom.subAndCancel = function (phoneInputTitle, phoneInputContent, attach, submitFn) {
    ...
    function goBack() {
        var $dwo = $('.dwo');
        var length = $dwo.length;
        if (length) {
            $dwo.triggerHandler('click');
        }
        else {
            validObj.isEdit = phoneInputTitle.isEdited() || phoneInputContent.isEdited() || validObj.isEdit;
            me.cancelValidate();
...
editCom.submit = function (page, data, ajaxUrl) {
    ...
    data = data || {};
    data.title = $.trim(me.formateEmoji($('#edit-title').text()));
    data.content = $.trim(me.formateEmoji($('#edit-content').html()));
