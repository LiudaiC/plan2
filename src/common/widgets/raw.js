...
raw.dateToDone = function (date, now) {
    ...
    if (!now) {
        ...
        var dateStart = util.dateformat(now).replace(/-/g, '/');

        // 修正到当天的 零点
        now = +new Date(dateStart + ' 00:00:00');
    }
    ...
    // 做周日的判断
    var day = dateInfo.dayOfWeek;
    if (day === 0) {
        day = lang.day;
    }

    if (weeksAway === 0) {
        return lang.currentWeekText + day
                + ' ' + dateInfo.h + ':' + dateInfo.m + ' '
                + lang.beforeText;
    }

    if (weeksAway === 1) {
        return lang.nextWeekText + day
                + ' ' + dateInfo.h + ':' + dateInfo.m
                + ' ' + lang.beforeText;
    }
